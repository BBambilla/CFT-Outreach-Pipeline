...
import { Mail, AlertTriangle, Clock, Trophy, Users, Plus, KanbanSquare, LayoutDashboard, ShieldAlert, Upload, MessageCircle, List, Search, Filter, CalendarDays, Users2, Flag, ArrowUpDown, Map, Download, Send, Settings } from 'lucide-react';
...
export const CoordinatorDashboard: React.FC = () => {
  ...
  const [view, setView] = useState<'overview' | 'leads' | 'pipeline' | 'reps' | 'followups' | 'admin' | 'bulk-email' | 'replies' | 'sent' | 'settings'>('overview');
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-3">
          <div className="flex flex-wrap bg-slate-100 p-1 rounded-lg border border-slate-200 gap-1">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${view === 'replies' ? 'bg-white text-brand-orange shadow-sm border border-slate-200/50' : 'text-slate-600 hover:text-slate-900'}`}
            >
              <Mail size={16} />
              Replies
            </button>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${view === 'settings' ? 'bg-white text-brand-orange shadow-sm border border-slate-200/50' : 'text-slate-600 hover:text-slate-900'}`}
            >
              <Settings size={16} />
              Settings
            </button>
